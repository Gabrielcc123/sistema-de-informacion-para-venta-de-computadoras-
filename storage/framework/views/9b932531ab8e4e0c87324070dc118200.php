...
<?php
    ...
    $ventasHoy = \App\Models\NotaVenta::where('fecha', $hoy)->sum('total') ?? 0;
    $ordenesPendientes = \App\Models\Orden::where('estado', 'Pendiente')->count();
    $usuariosActivos = \App\Models\Usuario::where('estado', 1)->count();
    $accionesHoy = \App\Models\Bitacora::whereDate('fecha', $hoy)->count();

    $ventasSemana = [];
    for ($i = 6; $i >= 0; $i--) {
        $fecha = now()->subDays($i)->toDateString();
        $label = now()->subDays($i)->translatedFormat('D');
        $ventasSemana[$label] = round(\App\Models\NotaVenta::where('fecha', $fecha)->sum('total') ?? 0, 2);
    }

    $pagosDistribucion = \App\Models\NotaVenta::selectRaw('idPago, COUNT(*) as total')
        ->groupBy('idPago')
        ->with('pago')
        ->get()
...
<?php endif; ?>
<?php /**PATH C:\Users\VICTUS\OneDrive\Desktop\Metodologia\Fase3 - bitacora , login , requisotos de password - copia\resources\views/livewire/dashboard/admin.blade.php ENDPATH**/ ?>