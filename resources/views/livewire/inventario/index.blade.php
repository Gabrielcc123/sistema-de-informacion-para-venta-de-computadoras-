...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public ?int $idProductoServicio = null;
    ...
    {
        return [
            'items' => ProductoServicio::with(['categoria', 'productoFisico'])
                ->when($this->search, function ($q) {
                    $q->where('nombre', 'like', "%{$this->search}%")
                      ->orWhereHas('categoria', fn($c) => $c->where('nombre', 'like', "%{$this->search}%"));
                })
                ->orderBy('idProductoServicio', 'desc')
                ->paginate(10),
    ...
    {
        $this->reset(['tipo', 'idCategoria', 'nombre', 'precioUnitario', 'garantia', 'stock', 'marca', 'modelo', 'numeroSerie']);
        $this->idProductoServicio = $id;
        $this->modoEdicion = !is_null($id);

        if ($this->modoEdicion) {
            $ps = ProductoServicio::with('productoFisico')->findOrFail($id);
            $this->tipo = $ps->tipo;
            $this->idCategoria = $ps->idCategoria;
            $this->nombre = $ps->nombre;
            $this->precioUnitario = $ps->precioUnitario;
            $this->garantia = $ps->garantia ?? '';

            if ($ps->productoFisico) {
                $this->stock = $ps->productoFisico->stock;
                $this->marca = $ps->productoFisico->marca ?? '';
                $this->modelo = $ps->productoFisico->modelo ?? '';
                $this->numeroSerie = $ps->productoFisico->numeroSerie ?? '';
            }
        } else {
            $this->tipo = 'Producto';
            $this->stock = null;
    ...
    public function cerrarModal(): void
    {
        ...
        $this->reset(['tipo', 'idCategoria', 'nombre', 'precioUnitario', 'garantia', 'stock', 'marca', 'modelo', 'numeroSerie', 'idProductoServicio', 'modoEdicion']);
    }
    ...
    public function guardar(): void
    {
        ...
        DB::transaction(function () {
            if ($this->modoEdicion) {
                $ps = ProductoServicio::findOrFail($this->idProductoServicio);
                $ps->update([
                ...
                if ($this->tipo === 'Producto') {
                    $producto = Producto::find($ps->idProductoServicio);
                    if ($producto) {
                        $producto->update([
                            'stock'       => $this->stock ?? 0,
                ...
                Bitacora::registrar('Item editado: ' . $this->nombre, Auth::id());
    ...
    public function eliminar(int $id): void
    {
        ...
        if ($ps->detalleVentas()->exists()) {
            $this->dispatch('notify', message: 'No se puede eliminar el registro porque tiene detalles de venta asociados.', type: 'error');
            return;
        }

        if ($ps->productoFisico) {
            $ps->productoFisico->delete();
        }
        $ps->delete();

        Bitacora::registrar('Item eliminado del inventario: ' . $ps->nombre, Auth::id());
                ...
                <tbody class="divide-y divide-gray-800 text-gray-300">
                    @forelse ($items as $item)
                        <tr class="hover:bg-[#262629] transition-colors">
                            ...
                            <td class="px-5 py-3 whitespace-nowrap text-gray-100">{{ $item->nombre }}</td>
                            <td class="px-5 py-3 whitespace-nowrap">{{ $item->categoria?->nombre ?? '—' }}</td>
                            <td class="px-5 py-3 whitespace-nowrap font-mono text-gray-100">Bs. {{ number_format($item->precioUnitario, 2) }}</td>
                            <td class="px-5 py-3 whitespace-nowrap">
                                @if ($item->tipo === 'Servicio')
                                    <span class="text-gray-500">—</span>
                                @else
                                    @php $stock = $item->productoFisico?->stock ?? 0; @endphp
                                    @if ($stock < 5)
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-red-400/10 text-red-400 border border-red-400/20">{{ $stock }} <span class="opacity-70">Stock Bajo</span></span>
                                    @else
                                        <span class="text-gray-100">{{ $stock }}</span>
                                    @endif
                                @endif
                            </td>
                            <td class="px-5 py-3 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button wire:click="abrirModal({{ $item->idProductoServicio }})"
                                    ...
                                    <button wire:click="eliminar({{ $item->idProductoServicio }})"
                                            wire:confirm="¿Está seguro de eliminar {{ $item->nombre }}?"
    ...
    @if($mostrarModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"
            ...
            <div class="bg-[#202022] border border-gray-800 rounded-xl p-6 w-full max-w-2xl shadow-2xl mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <p class="text-sm text-gray-400 mb-5">Complete los datos del {{ $modoEdicion ? 'registro' : 'producto o servicio' }}.</p>
